...
const (
	FirewallsorceressMaxAttacksLoop    = 40
	minFirewallSorceressAttackDistance = 0
	maxFirewallSorceressAttackDistance = 2
	dangerDistanceFirewall             = 0 // Monsters closer than this are considered dangerous
	safeDistanceFirewall               = 1 // Distance to teleport away to
)

type FirewallSorceress struct {
	BaseCharacter
}

func (s FirewallSorceress) isPlayerDead2() bool {
	return s.Data.PlayerUnit.HPPercent() <= 0
}

func (s FirewallSorceress) CheckKeyBindings() []skill.ID {
	requireKeybindings := []skill.ID{skill.FireWall, skill.Teleport, skill.TomeOfTownPortal}
...
func (s FirewallSorceress) KillMonsterSequence(
	...
	attackOpts := step.StationaryDistance(minFirewallSorceressAttackDistance, maxFirewallSorceressAttackDistance)
	...
	for {
		...
		if completedAttackLoops >= FirewallsorceressMaxAttacksLoop {
			return nil
		}
		...
		step.SecondaryAttack(skill.FireWall, id, 1, attackOpts)
...
func (s FirewallSorceress) killMonster(npc npc.ID, t data.MonsterType) error {
...
func (s FirewallSorceress) killMonsterByName(id npc.ID, monsterType data.MonsterType, skipOnImmunities []stat.Resist) error {
...
func (s FirewallSorceress) BuffSkills() []skill.ID {
...
/*
func (s FirewallSorceress) KillMephisto() error {
    // Find Mephisto
    ...
    // Now, proceed with the regular monster killing sequence (Firewall etc.)
...
func (s FirewallSorceress) KillMephisto() error {
	...
	for attackCount < maxAttack {
		...
		step.SecondaryAttack(skill.FireWall, monster.UnitID, 1, opts)
		utils.Sleep(50)
		attackCount++
	}
...
func (s FirewallSorceress) KillIzual() error {
	m, _ := s.Data.Monsters.FindOne(npc.Izual, data.MonsterTypeUnique)
	_ = step.SecondaryAttack(skill.FireWall, m.UnitID, 4, step.Distance(5, 8))

	return s.killMonsterByName(npc.Izual, data.MonsterTypeUnique, nil)
}

func (s FirewallSorceress) KillDiablo() error {
	...
	for {
		...
		_ = step.SecondaryAttack(skill.FireWall, diablo.UnitID, 5, step.Distance(3, 8))

		return s.killMonsterByName(npc.Diablo, data.MonsterTypeUnique, nil)
	}
}

func (s FirewallSorceress) KillPindle() error {
...
func (s FirewallSorceress) KillNihlathak() error {
	return s.killMonsterByName(npc.Nihlathak, data.MonsterTypeSuperUnique, nil)
}

func (s FirewallSorceress) KillBaal() error {
	m, _ := s.Data.Monsters.FindOne(npc.BaalCrab, data.MonsterTypeUnique)
	step.SecondaryAttack(skill.FireWall, m.UnitID, 4, step.Distance(5, 8))

	return s.killMonsterByName(npc.BaalCrab, data.MonsterTypeUnique, nil)
}

func (s FirewallSorceress) needsRepositioning() (bool, data.Monster) {
	for _, monster := range s.Data.Monsters.Enemies() {
		...
		if distance < dangerDistanceFirewall {
			return true, monster
		}
	}
...
func (s FirewallSorceress) findSafePosition(targetMonster data.Monster) (data.Position, bool) {
	ctx := context.Get()
	...
	if length > 0 {
		normalizedX := int(float64(vectorX) / length * float64(safeDistanceFirewall))
		normalizedY := int(float64(vectorY) / length * float64(safeDistanceFirewall))
	...
	for angle := 0; angle < 360; angle += 5 {
		...
		for distance := minSafeMonsterDistance; distance <= safeDistanceFirewall+3; distance += 2 {
	...
	for _, pos := range candidatePositions {
		...
		if targetDistance >= minFirewallSorceressAttackDistance && targetDistance <= maxFirewallSorceressAttackDistance {
			attackRangeScore = 10.0
		} else {
			// Penalize positions outside attack range
			attackRangeScore = -math.Abs(float64(targetDistance) - float64(minFirewallSorceressAttackDistance+maxFirewallSorceressAttackDistance)/2.0)
		}
		...
		if minMonsterDistance > float64(dangerDistanceFirewall) {
			score += 5.0
		}
	...
	if len(scoredPositions) > 0 {
		s.Logger.Info(fmt.Sprintf("Found safe position with score %.2f at distance %.2f from nearest monster",
			scoredPositions[0].score, minMonsterDistanceFirewall(scoredPositions[0].pos, s.Data.Monsters)))
...
func minMonsterDistanceFirewall(pos data.Position, monsters data.Monsters) float64 {
